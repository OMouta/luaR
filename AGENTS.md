...
`std/` holds it, one LuaR module per file, compiled into `luarc`. `std/mem` is `std/mem.luar`. A collection, optional, or `Result` method written in LuaR lives in `std/prelude.luar`; the compiler keeps only what has no LuaR spelling.
